		...
		//Draw it!
			...
			if(stm.showAppearanceFoldout){
				...
				if(GUILayout.Button("New"))
                {
                    ...
                    if(!AssetDatabase.IsValidFolder(ClavianPath + "Materials")){
                        //create folder if it doesn't exist yet
                        AssetDatabase.CreateFolder(ClavianPath.Remove(ClavianPath.Length - 1), "Materials");
                    }
                    AssetDatabase.CreateAsset(newMaterial, AssetDatabase.GenerateUniqueAssetPath(ClavianPath + "Materials/NewMaterial.mat"));
			...
			if(stm.showEventFoldout){
				...
				EditorGUILayout.PropertyField(serializedObject.FindProperty("customEvent"));
...
public class SuperTextMesh : MonoBehaviour, ILayoutElement { //MaskableGraphic... rip
	...
	public bool uiWrap = false;
	[Tooltip("If true, STM will set its bounds based on RectTransform, without need for Content Size Fitter.")]
	public bool uiLimit = false;
...
*/
	...
	*/
	...
	private void ShowAllText(bool unreadingMesh){
		...
		StopReadRoutine();
		SetMesh(totalReadTime, unreadingMesh);
...
//			alreadyInvoked.Clear();
	...
	*/
	...
	string ParseText(string myText)
    {
		...
		if(preParse != null && preParse.GetPersistentEventCount() > 0)
        {
			STMTextContainer tempContainer = new STMTextContainer(myText);
			preParse.Invoke(tempContainer);
...
*/
	...
	void RecalculateTextBounds(){
		if(hyphenedText.Length > 0)
		{
			...
			//TODO: okay this doesn't work perfect for bottom-anchored text just yet. why
			float textBoundsBottom = Mathf.Max(lowestDrawnPosition, lowestPosition - rawTopLeftBounds.y);
			//line up with text...
...
Vector3 JitterValue_MyJit = Vector3.zero;
   ...
   //--------------------------------------------------
	...
	{ //set the data for the endmesh
        ...
        if (vertexMod != null && vertexMod.GetPersistentEventCount() > 0){
			...
			vertexMod.Invoke(endVerts, UpdateMesh_Middles, UpdateMesh_Positions); //modify end verts externally
